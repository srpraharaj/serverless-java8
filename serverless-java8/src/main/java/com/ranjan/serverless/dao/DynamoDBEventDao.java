package com.ranjan.serverless.dao;
...
import com.ranjan.serverless.manager.DynamoDBManager;
...
public class DynamoDBEventDao implements EventDao {
	...
	public List<Event> findEventsByCity(String city) {
		...
		DynamoDBQueryExpression<Event> query = new DynamoDBQueryExpression<Event>().withIndexName(Event.CITY_INDEX)
				.withConsistentRead(false).withKeyConditionExpression("city = :v1").withExpressionAttributeValues(eav);
	...
	public List<Event> findEventsByTeam(String team) {
		...
		DynamoDBQueryExpression<Event> awayQuery = new DynamoDBQueryExpression<Event>()
				.withIndexName(Event.AWAY_TEAM_INDEX).withConsistentRead(false)
				.withKeyConditionExpression("awayTeam = :v1").withExpressionAttributeValues(eav);
